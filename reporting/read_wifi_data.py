...
for key in r.scan_iter():
    ...
    if 'energy_segment' in str(key):
        ...
        for idx in range(list_length-1, 0, -1):
            ...
            try:
                wifi_ap_name = segment.wifi_ap_name
                wifi_signal_strength = segment.wifi_signal_strength
                print(wifi_ap_name, wifi_signal_strength)
                if wifi_ap_name not in results.keys():
                    results[wifi_ap_name] = []
                results[wifi_ap_name].append(wifi_signal_strength)
            except:
                break
