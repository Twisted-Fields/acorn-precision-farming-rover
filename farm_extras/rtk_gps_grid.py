import sys
sys.path.append("/home/acorn/vehicle")
import time
import gps
import gps_tools
import subprocess
...
# allows setting of the orientation of the grid by drawing a line in our
# our acorn debug interface, to align with roads or other map features
USE_DATABASE = False
...
if not USE_DATABASE:
    gps_module = gps.GPS(logger, simulate_hardware=False, single_gps=True, single_gps_port="/dev/ttyACM0")
...
while True:
    ...
    error_counter = 0
    # if print_values:
    #     print(f"Diff soln: {gps_module.raw_samples.difSoln==1}")
...
#    sample_tick+=1
    ...
    if latest_sample.status == True and latest_sample.rtk_age < 20 and gps_module.raw_samples.difSoln==1:
        recent_distance_list.append((distance, time.time()))

        while time.time() - recent_distance_list[0][1] > 60:
            recent_distance_list.pop(0)

        distances = [t[0] for t in recent_distance_list]
        total_error = max(distances) - min(distances)
